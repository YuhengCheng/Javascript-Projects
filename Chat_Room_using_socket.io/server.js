...
io.on('connection', function(socket) {
  ...
  //once user enters valid username, join them to chat room to see messages and notify that user has connected
  ...
  socket.on('clientSays', function(data) {
    ...
    //broadcasts to everyone in room except for original sender, this enables text coloring
    socket.broadcast.to('room1').emit('serverSays', data)
  })
  //private message
  socket.on('privMsg', function(data) {
    //emit only to user based on username
    socket.to(data.user).emit('serverPrivMsg', data.msg)
  })
